...
import { formatCurrency, formatDate, formatDateTime, getStatusLabel, getStatusColor, STATUS_STEPS } from '@/lib/utils'

interface Application {
  id: string; referenceNo: string; status: string
  fullName: string; email: string; phone: string; county: string
  idNumber: string; dateOfBirth: string; gender: string
  institutionName: string; institutionType: string; course: string
  yearOfStudy: number; admissionNumber: string
  feesRequired: number; amountRequested: number; otherFunding: number
  personalStatement: string; guardianName: string; guardianOccupation: string
  householdIncome: number; dependents: number; isOrphan: boolean
  hasDisability: boolean; createdAt: string; updatedAt: string
  adminComment?: string
  payment?: { status: string; mpesaReceiptNo?: string; amount: number; createdAt: string }
  documents?: Array<{ id: string; documentType: string; fileName: string; filePath: string }>
  ...
  if (!application) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/dashboard" className="text-brand-600 text-sm hover:underline mt-2 inline-block">
          Back to dashboard
        </Link>
      </div>
    )
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-xs font-semibold px-3 py-1.5 rounded-full border ${getStatusColor(application.status)}`}>
            ...
            {STATUS_STEPS.map((step, i) => (
              <div key={step} className="relative flex flex-col items-center z-10">
                <div className={`w-9 h-9 rounded-full border-2 flex items-center justify-center transition-all ${
                  i < statusIndex ? 'bg-brand-600 border-brand-600' :
                  i === statusIndex ? 'bg-white border-brand-600 ring-4 ring-brand-100' :
                ...
                <span className={`mt-2 text-xs font-semibold whitespace-nowrap hidden sm:block ${
      ...
      {application.adminComment && (
        <div className={`rounded-2xl border p-5 ${
          application.status === 'APPROVED' ? 'bg-emerald-50 border-emerald-200' :
          application.status === 'REJECTED' ? 'bg-red-50 border-red-200' :
          'bg-blue-50 border-blue-200'
        }`}>
          <p className={`text-sm font-bold mb-1 ${
            application.status === 'APPROVED' ? 'text-emerald-800' :
          ...
          <p className={`text-sm ${
            ...
            application.status === 'REJECTED' ? 'text-red-700' : 'text-blue-700'
          }`}>
            {application.adminComment}
          </p>
        </div>
      ...
      <div className="grid gap-5">
        ...
        <SectionCard title="Financial Details" icon={DollarSign}>
          <FieldGrid fields={[
            { label: 'Total Fees Required', value: formatCurrency(application.feesRequired) },
            ...
            { label: 'Household Income', value: `${formatCurrency(application.householdIncome)}/month` },
        ...
        {application.payment && (
          <SectionCard title="Payment" icon={CheckCircle}>
            <FieldGrid fields={[
              ...
              { label: 'M-Pesa Receipt', value: application.payment.mpesaReceiptNo || '—' },
              ...
              {application.statusLogs.map((log, i) => (
                <div key={i} className="flex items-start gap-3">
                  <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${
                    log.status === 'APPROVED' || log.status === 'DISBURSED' ? 'bg-emerald-500' :
                    log.status === 'REJECTED' ? 'bg-red-500' : 'bg-brand-500'
                  }`} />
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{getStatusLabel(log.status)}</p>
